...
const AppState = ({ initial, children }: AppProps) => {
    ...
    const confirmEvent = (event: ProcessedEvent, action: EventActions) => {
        ...
        if (action === 'edit') {
            updatedEvents = state.events.map(e => (e._id === event._id ? event : e));
        } else {
    ...
    const onDrop = async (eventId: string, startTime: Date, resKey: string, resVal: string | number) => {
        // Get dropped event
        const droppedEvent = state.events.find(e => {
            if (typeof e._id === 'number') {
